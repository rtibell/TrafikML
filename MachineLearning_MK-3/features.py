...
def continuous_features(day_nr, month_of_year, minutes_since_daybreak, days_until_holiday, days_scaler) -> np.ndarray:
    ...
    scaled daysUntilHoliday. `holidayType` and `holidayNum` are *not* included here --
    they are small-cardinality categoricals, embedded separately by the model (see
    model.py::TabularMLPClassifier). `statusEnum` is the classification target in this
    version and isn't a feature at all -- see dataset.py and Solution.md.
    """
