...
- **Early stopping**: training halts once validation *balanced accuracy* (macro-average
  ...
  because `statusEnum` is heavily imbalanced (~88% freeflow): raw accuracy rewards a
  model that just always predicts freeflow, balanced accuracy does not.
...
`torch.softmax` + `argmax` to turn them into a predicted `statusEnum`, used below for
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="'balanced' (default) weights the cross-entropy loss inversely to training-set class frequency -- "
             "statusEnum is heavily imbalanced (~88%% freeflow, ~0.3%% impossible), see Solution.md",
    )
...
def run_epoch(model, loader, device, ce_loss, optimizer=None, grad_clip: float = 5.0) -> dict:
    ...
    # always predicts the majority class (freeflow, ~88% of rows -- see Solution.md)
    # scores ~0.25 here instead of ~0.88, so it -- not raw accuracy -- is what
...
def run_training(args: argparse.Namespace) -> dict:
    ...
    validation accuracy, whether early stopping fired, the checkpoint path (if any) and
    the full epoch history. Shared by train.py's CLI and tune.py's search loop --
    tune.py passes `checkpoint_dir=None` to skip writing checkpoints for trials it
    isn't keeping.
    """
    set_seed(args.seed)
