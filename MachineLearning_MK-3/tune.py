...
batch size, embedding sizes, class-weighting), reusing train.py's `run_training` for
every trial (short epoch budget, early stopping still active, no checkpoint written) so
ranking uses exactly the same training code path as a standalone `train.py` run. The
winning configuration (by best validation *balanced* accuracy -- see train.py's
module docstring for why raw accuracy is the wrong ranking metric on this imbalanced
target) is then retrained for a full epoch budget and checkpointed.
...
def sample_config(rng: random.Random) -> dict:
    return {
        ...
        "class_weights": rng.choice(["balanced", "none"]),
    }
...
def main() -> None:
    ...
    for trial in range(1, args.trials + 1):
        ...
        row = {**config, "trial": trial, "val_balanced_accuracy": result["best_val_balanced_accuracy"], "stopped_epoch": result["stopped_epoch"] or args.trial_epochs}
        results.append(row)
        print(f"trial {trial:3d}/{args.trials}  val_bal_acc={row['val_balanced_accuracy']:.4f}  stopped@{row['stopped_epoch']:3d}  {config}")
        if best is None or row["val_balanced_accuracy"] > best["val_balanced_accuracy"]:
            best = row
    ...
    final_config = {k: v for k, v in best.items() if k not in ("trial", "val_balanced_accuracy", "stopped_epoch")}
    ...
    print(f"Final model val balanced accuracy: {final_result['best_val_balanced_accuracy']:.4f}. Checkpoint: {final_checkpoint_dir}/best.pt")
