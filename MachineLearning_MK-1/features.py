"""Feature engineering shared by dataset building, training and evaluation.
...
def calendar_features(day_nr, month_of_year, minutes_since_daybreak, days_until_holiday, days_scaler):
    """Vectorized calendar/holiday covariates that are known ahead of time for any
    future timestamp (they depend only on the calendar date, never on a measurement).

    Returns an (N, 7) float32 array: sin/cos(dayNr), sin/cos(month), sin/cos(minutes),
    scaled daysUntilHoliday.
    """
...
# Number of columns produced by calendar_features()
CALENDAR_FEATURE_DIM = 7
