...
batch size, class-weighting), reusing train.py's `run_training` for every trial (short
epoch budget, early stopping still active, no checkpoint written) so ranking uses
exactly the same training code path as a standalone `train.py` run. The winning
configuration (by best validation *balanced* accuracy -- see train.py's module
