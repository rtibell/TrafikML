# Operations — running MachineLearning_MK-5

How to set up, get data, train, tune, and evaluate the MLP status classifier. See
...
`SegmentSpeed` records in the revised one-hot schema — see `Solution.md`'s Data
contract section, or `REST-opperations.md` in the repo root, for the exact fields. Only
the four `*_status` fields (the target) and the non-`speed` covariates
(`dayNr`/`daysUntilHoliday`/`minutesSincDaybreak`/`monthOfYear`/`holiday_type_*`/
`holiday_is_*`) are used; `speed`, `sectionId`, `measureTime`, `status`, and
`statusEnum` are read but ignored — see Solution.md for why.

> **If `raw-trafic-data.json` is missing the one-hot fields** (an old export, or one
> from before the backend's schema change), `dataset.py::load_dataframe` raises
> `ValueError: ... is missing expected fields: [...]` immediately rather than silently
> training on the wrong columns. Re-export it (Option A below).
...
JSON array. Identical to the earlier `MK-*` versions' script of the same name — it
doesn't need to know or care which fields are the target vs. inputs, since it just
...
training starts, then one line per epoch with the current LR, and train/val loss, raw
accuracy, and **balanced accuracy** (macro-average recall — the metric that actually
drives early stopping/checkpointing; see Solution.md for why raw accuracy alone is
misleading on this imbalanced target). Expect balanced accuracy to be noisier
epoch-to-epoch than the loss, especially with `--class-weights balanced`.
...
export — `sigmoid` is clearly worst, `tanh`/`leaky_relu` lead.
...
python train.py --data synthetic-trafic-data.json --epochs 10 --batch-size 128 \
    --num-layers 2 --hidden-dim 32 --checkpoint-dir /tmp/mk5-smoke
python evaluate.py --checkpoint /tmp/mk5-smoke/best.pt --data synthetic-trafic-data.json \
    --output-dir /tmp/mk5-smoke/eval
```

This was run as part of building this version (8,000 synthetic rows, every
`--activation` choice, and `tune.py` end-to-end) on CPU, and against the real
...
|---|---|
| `ValueError: raw-trafic-data.json is missing expected fields` | File doesn't match the revised one-hot `SegmentSpeed` schema — check for `holiday_type_*`/`holiday_is_*`/`*_status` specifically; an export from before the backend's schema change (or from `MK-3`'s old integer-only shape) will be missing all of them |
...
| `impossible`/`congested` precision or recall is 0 or looks unstable across runs | These classes have very little support (1,084 and 11,252 rows total respectively, out of 438,847) — see Solution.md's caveat on small-sample metrics for the rarest classes |
