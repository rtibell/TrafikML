...
REST-opperations.md: `holiday_type_*`/`holiday_is_*` arrive as upstream one-hot fields
(no embedding tables needed -- see Solution.md), and `freeflow_status`/`heavy_status`/
`congested_status`/`imposible_status` are this version's **target** (see dataset.py),
not inputs -- unlike `MachineLearning_MK-4`, which used them as inputs to regress
`speed`.
"""
...
STATUS_CARDINALITY = 4  # freeflow=0, heavy=1, congested=2, impossible=3

# The one-hot target fields, in `STATUS_VALUES` order -- argmax over these columns
# (dataset.py::derive_status_class) gives the integer class index used everywhere else.
STATUS_FIELDS = ["freeflow_status", "heavy_status", "congested_status", "imposible_status"]

# Input one-hot field groups, in the order given in the task's SegmentSpeed feature
...
ONE_HOT_INPUT_FIELDS = HOLIDAY_TYPE_FIELDS + HOLIDAY_IS_FIELDS
ONE_HOT_FEATURE_DIM = len(ONE_HOT_INPUT_FIELDS)  # 13
...
def one_hot_features(rows) -> np.ndarray:
    """Pass the 13 upstream `holiday_type_*`/`holiday_is_*` input fields straight
    through as an (N, 13) float32 array -- already 0/1, so no further encoding is
    needed on this side. `STATUS_FIELDS` are deliberately *not* included here -- they
    are this version's target, see dataset.py::derive_status_class.
    """
    return rows[ONE_HOT_INPUT_FIELDS].to_numpy(dtype=np.float32)
...
def continuous_features(day_nr, month_of_year, minutes_since_daybreak, days_until_holiday, days_scaler) -> np.ndarray:
    ...
    scaled daysUntilHoliday. See `one_hot_features` above for the other 13 columns of
    the model's full input vector (`features.py::FEATURE_DIM`).
    """
...
# Full model input width: one_hot_features() (13) ++ continuous_features() (7)
FEATURE_DIM = ONE_HOT_FEATURE_DIM + CONTINUOUS_FEATURE_DIM  # 20
